...
simParameters.NFrames = 20;      % Number of 10 ms frames
simParameters.SNRIn = [-5 0 5 10 15 20 25 30 35 40]; % SNR range (dB) [-10 10 20 30 40 50]
...
simParameters.PDSCH.DMRS.DMRSLength              = 2; % %Mudei de 2 para 1;;; Number of front-loaded DM-RS symbols (1(single symbol),2(double symbol))
...
simParameters.PDSCH.DMRS.NumCDMGroupsWithoutData = 3; % CDM groups without data (1,2,3) mudei de 3 para 2
...
%simParameters.TransmitAntennaArray.PanelDimensions  = [8 2]; % Number of columns and rows in the transmit panel (N1, N2) (38tx)
...
%simParameters.CSIRS.RowNumber = 16; % 1...18 (32tx)
...
%simParameters.CSIRS.SymbolLocations = [12 13];% (32Tx)
...
simParameters.CSIReportMode = 'AI CSI compression'; % 'RI-PMI-CQI','AI CSI compression' (Feedback Mode)
...
if simParameters.CSIReportMode == "RI-PMI-CQI"  
    ...
    % File name of the AI neural network
    
    
    simParameters.AINetworkFilename = 'dCSITrainedNetwork_C2_12_03_02_23.mat';
    simParameters.AIPrecoderRegMode = 'adaptive';
    simParameters.AIPrecoderRegAlpha = [0.03, 0.5]; 
    %  [alpha_floor, alpha_scale]
    %  alpha = 0.01 + 0.5 * nVar
    simParameters.AINoiseScaleMode = 'adaptive';
    simParameters.AINoiseScaleParams = [0.08, 0.40, 15, 0.20];
    simParameters.AIRankDiscount = 0.10; %0.15
...
% =====================================================================
maxNumLayers = 8;
NRB = simParameters.Carrier.NSizeGrid;
evmAllSNR = struct();  % Estrutura principal de EVM
...
for snrIdx = 1:numel(simParameters.SNRIn)
    ...
    % Arrays for this SNR point
    evmPerSlotLocal = NaN(NSlots, maxNumLayers);
    evmPerRBLocal   = NaN(NSlots, NRB, maxNumLayers);
    numLayersPerSlot = zeros(NSlots, 1);
    ...
    for nslot = 0:NSlots-1
        ...
        % MODIFICADO: agora retorna valores numéricos
        if (simParamLocal.DisplayDiagnostics)
            [evmSlot, evmRB] = plotLayerEVM(NSlots,nslot,pdsch,size(dlGrid),pdschIndices,pdschSymbols,pdschEq);
            
            % Armazenar nos arrays locais
            evmPerSlotLocal(nslot+1, :) = evmSlot(nslot+1, :);
            evmPerRBLocal(nslot+1, :, :) = evmRB;
            numLayersPerSlot(nslot+1) = pdsch.NumLayers;
        end
        ...
        if csirsTransmission
            ...
            %rxCSIReport = hCSIEncode_SNR(carrier,csirs,Hest,noiseEst,csiFeedbackOpts);
    ...
    % =====================================================================
    % EVM forthe current SNR point
    % =====================================================================
    evmAllSNR(snrIdx).SNRdB           = simParameters.SNRIn(snrIdx);
    evmAllSNR(snrIdx).evmPerSlot      = evmPerSlotLocal;       % [NSlots x 8]
    evmAllSNR(snrIdx).evmPerRB        = evmPerRBLocal;          % [NSlots x NRB x 8]
    evmAllSNR(snrIdx).numLayersPerSlot = numLayersPerSlot;      % [NSlots x 1]
    
    % EVM statistics ANALYSIS
    activeEVM = evmPerSlotLocal(:, 1);  % Layer 1 sempre ativo
    evmAllSNR(snrIdx).meanEVM    = nanmean(evmPerSlotLocal, 1);  % [1 x 8]
    evmAllSNR(snrIdx).medianEVM  = nanmedian(evmPerSlotLocal, 1);
    evmAllSNR(snrIdx).maxEVM     = nanmax(evmPerSlotLocal, [], 1);
    evmAllSNR(snrIdx).stdEVM     = nanstd(evmPerSlotLocal, 0, 1);
    evmAllSNR(snrIdx).p5EVM      = prctile(evmPerSlotLocal, 5, 1);
    evmAllSNR(snrIdx).p95EVM     = prctile(evmPerSlotLocal, 95, 1);
    evmAllSNR(snrIdx).meanEVMperRB = squeeze(nanmean(evmPerRBLocal, 1)); % [NRB x 8]
    ...
    % Imprimir resumo de EVM
    fprintf('EVM Layer1: mean=%.2f%%, median=%.2f%%, p95=%.2f%%, max=%.2f%%\n', ...
        evmAllSNR(snrIdx).meanEVM(1), evmAllSNR(snrIdx).medianEVM(1), ...
        evmAllSNR(snrIdx).p95EVM(1), evmAllSNR(snrIdx).maxEVM(1));
...
% =====================================================================
%  GRÁFICOS QUANTITATIVOS DE EVM
% =====================================================================
plotEVMAnalysis(simParameters, evmAllSNR);
...
simResults.evmAllSNR = evmAllSNR;  

%Imprimir tabela resumo de EVM por SNR
fprintf('\n\n========== RESUMO EVM POR SNR ==========\n');
fprintf('%6s | %10s | %10s | %10s | %10s | %10s\n', ...
    'SNR', 'Mean(%)', 'Median(%)', 'P5(%)', 'P95(%)', 'Max(%)');
fprintf('-------|------------|------------|------------|------------|------------\n');
for s = 1:numel(simParameters.SNRIn)
    fprintf('%4ddB | %10.2f | %10.2f | %10.2f | %10.2f | %10.2f\n', ...
        simParameters.SNRIn(s), ...
        evmAllSNR(s).meanEVM(1), ...
        evmAllSNR(s).medianEVM(1), ...
        evmAllSNR(s).p5EVM(1), ...
        evmAllSNR(s).p95EVM(1), ...
        evmAllSNR(s).maxEVM(1));
end
fprintf('========================================\n');


% =====================================================================
%  FUNÇÃO DE ANÁLISE E PLOT DE EVM
% =====================================================================
function plotEVMAnalysis(simParameters, evmAllSNR)


    SNRs = simParameters.SNRIn;
    nSNR = numel(SNRs);
    
    
    meanEVM   = arrayfun(@(x) x.meanEVM(1), evmAllSNR);
    medianEVM = arrayfun(@(x) x.medianEVM(1), evmAllSNR);
    p5EVM     = arrayfun(@(x) x.p5EVM(1), evmAllSNR);
    p95EVM    = arrayfun(@(x) x.p95EVM(1), evmAllSNR);
    maxEVM    = arrayfun(@(x) x.maxEVM(1), evmAllSNR);
    
    % --- EVM médio vs SNR com intervalo de confiança ---
    figure('Name','EVM Analysis','Position',[100 100 900 700]);
    
    subplot(2,2,1)
    hold on; grid on;
    fill([SNRs fliplr(SNRs)], [p5EVM fliplr(p95EVM)], ...
        [0.8 0.9 1.0], 'EdgeColor','none', 'FaceAlpha', 0.5);
    plot(SNRs, meanEVM, 'b-o', 'LineWidth', 1.5, 'MarkerSize', 6);
    plot(SNRs, medianEVM, 'r--s', 'LineWidth', 1.2, 'MarkerSize', 5);
    xlabel('SNR (dB)'); ylabel('EVM (%)');
    title('EVM vs SNR (Layer 1)');
    legend('P5-P95 range', 'Mean', 'Median', 'Location', 'northeast');
    
    % --- Gráfico 2: EVM máximo vs SNR ---
    subplot(2,2,2)
    plot(SNRs, maxEVM, 'r-^', 'LineWidth', 1.5);
    hold on; grid on;
    plot(SNRs, p95EVM, 'b-v', 'LineWidth', 1.2);
    plot(SNRs, meanEVM, 'k-o', 'LineWidth', 1.2);
    xlabel('SNR (dB)'); ylabel('EVM (%)');
    title('EVM Peak & Percentile vs SNR');
    legend('Max', 'P95', 'Mean', 'Location', 'northeast');
    
    % --- Gráfico 3: EVM por RB (perfil em frequência) para SNRs selecionados ---
    subplot(2,2,3)
    hold on; grid on;
    snrPlotIdx = round(linspace(1, nSNR, min(5, nSNR)));  % Selecionar até 5 SNRs
    colors = lines(length(snrPlotIdx));
    legendStr = {};
    for ii = 1:length(snrPlotIdx)
        s = snrPlotIdx(ii);
        rbProfile = evmAllSNR(s).meanEVMperRB(:, 1);  % Layer 1
        plot(0:length(rbProfile)-1, rbProfile, '-', 'Color', colors(ii,:), 'LineWidth', 1.2);
        legendStr{end+1} = sprintf('%d dB', SNRs(s)); %#ok<AGROW>
    end
    xlabel('Resource Block'); ylabel('EVM (%)');
    title('Mean EVM per RB (Layer 1)');
    legend(legendStr, 'Location', 'northeast');
    
    % --- Gráfico 4: CDF do EVM para SNRs selecionados ---
    subplot(2,2,4)
    hold on; grid on;
    legendStr = {};
    for ii = 1:length(snrPlotIdx)
        s = snrPlotIdx(ii);
        evmData = evmAllSNR(s).evmPerSlot(:, 1);
        evmData = evmData(~isnan(evmData));
        [f, x] = ecdf(evmData);
        plot(x, f, '-', 'Color', colors(ii,:), 'LineWidth', 1.5);
        legendStr{end+1} = sprintf('%d dB', SNRs(s)); %#ok<AGROW>
    end
    xlabel('EVM (%)'); ylabel('CDF');
    title('CDF of EVM per Slot (Layer 1)');
    legend(legendStr, 'Location', 'southeast');
    
    sgtitle(sprintf('EVM Analysis - %s (%dx%d) / CSI: %s', ...
        simParameters.DelayProfile, simParameters.NTxAnts, simParameters.NRxAnts, ...
        char(simParameters.CSIReportMode)));
end
...
% initial CSI feedback from perfect channel knowledge.
    ...
    csiFeedbackOpts.PerfectChannelEstimator = simParameters.PerfectChannelEstimator; %era true antes
...
% Create a CSI feedback algorithmic options structure
    ...
    if simParameters.CSIReportMode == "AI CSI compression"
        % additional link adaptation configuration for AI CSI compression mode
        csiFeedbackOpts.AINetworkFilename = simParameters.AINetworkFilename;

         if isfield(simParameters, 'AINoiseScaleFactor')
            csiFeedbackOpts.AINoiseScaleFactor = simParameters.AINoiseScaleFactor; 
         end

         % Noise scaling
        if isfield(simParameters,'AINoiseScaleMode')
            csiFeedbackOpts.AINoiseScaleMode = simParameters.AINoiseScaleMode;
        end

        if isfield(simParameters,'AINoiseScaleParams')
            csiFeedbackOpts.AINoiseScaleParams = simParameters.AINoiseScaleParams;
        end

        if isfield(simParameters, 'AIRankDiscount')
            csiFeedbackOpts.AIRankDiscount = simParameters.AIRankDiscount;%if adicionado para tratamento de ruído
        end 

        % Precoder regularization 
        if isfield(simParameters,'AIPrecoderRegMode')
            csiFeedbackOpts.AIPrecoderRegMode = simParameters.AIPrecoderRegMode;
        end
        if isfield(simParameters,'AIPrecoderRegAlpha')
            csiFeedbackOpts.AIPrecoderRegAlpha = simParameters.AIPrecoderRegAlpha;
        end
...
function [evmSlotData, evmRBData] = plotLayerEVM(NSlots,nslot,pdsch,siz,pdschIndices,pdschSymbols,pdschEqSymbols)
% Plot EVM information and return numerical EVM data
%
% Outputs:
%   evmSlotData - [NSlots x 8] EVM (%) per slot per layer (acumulado)
%   evmRBData   - [NRB x 8]   EVM (%) per RB per layer (slot atual)
    ...
    evmSlotData = evmPerSlot;   % [NSlots x 8] - histórico completo
    evmRBData   = evmPerRB;     % [NRB x 8]    - snapshot do slot atual
end
